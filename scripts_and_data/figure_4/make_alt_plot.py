...
draw_label(ax, "Rapid rearrangement ($\it{Type}$ $\it{I}$)")
...
ax.text(0.5, 0.6, r"$RI$=" + str(adjustment_ri)[:4] + r"$\pm$" + str(adjustment_ri_std)[:4], horizontalalignment='center')
plt.axis("off")

ax = fig.add_axes([middle, top+height, 0.2, landscape_height])
ax.arrow(0.1, 0.5, 0.8, 0, head_width=0.1, head_length=0.05, fc='k', ec='k')
ax.text(0.5, 0.6, r"$RI$=" + str(rearrangement_ri)[:4] + r"$\pm$" + str(rearrangement_ri_std)[:4], horizontalalignment='center')
plt.axis("off")
