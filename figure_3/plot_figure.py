...
def draw_label(ax, label):
	ax.text(0.5, 0.5, label, ha="center", fontsize=30)
	ax.axis("off")
...
while True:
	...
	ax = fig.add_axes([left_border, 0.05, width, height])
	...
	#ax = fig.add_axes([left_border, 0.35, width, 0.1])
	#state = draw_landscape(ax, before_totals, "before")
	if state=="bad":
	...
	ax.set_title("Slow adjustment", fontsize=24)
	ttl = ax.title
...
new_organisms, new_abundances = brute_force_rearrangement(after_totals, n_species, organisms, 30)
new_colors = generate_colors(len(new_organisms)-2, new=True) + colors[:2]
ax = fig.add_axes([0.6, 0.05, width, height])
...
ax = fig.add_axes([0.4, 0.05, 0.2, height])
draw_connections(ax, before_coord, after_coord, colors)
draw_connections(ax, rear_before, rear_after, new_colors)
ax.set_ylim(0, sum(abundances)+3)
ax.set_title("Rapid rearrangement", fontsize=24)
ttl = ax.title
ttl.set_position([.5, 1.0])
