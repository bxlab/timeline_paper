...
def re_standardize_contigs_by_taxonomy(depths, samples, contigs, taxonomy, taxa):
        print "re-standardizing contig depths for only "+taxa
        for sample in samples:
                total=0
                for contig in contigs:
...
def plot_even_clustermap(df):
	...
	g = sns.clustermap(df, col_cluster=True, col_colors=lut, cmap="Blues", figsize=(12,10), vmin=0, vmax=1)
...
def select_paths_from_list(pathway_abundances, list_file):
	print "selecting only pathways in "+list_file
	paths={}
...
if len(sys.argv)>1: 
	function_carriers = select_taxonomy_contigs(function_carriers, taxonomy, sys.argv[1])
	depths = re_standardize_contigs_by_taxonomy(depths, samples, contigs, taxonomy, sys.argv[1])
pathway_abundances = get_pathway_abundances(function_carriers, depths)
relevant_pathways = select_paths_from_list(pathway_abundances, "differential_paths.list")
plot_even_clustermap(relevant_pathways)
